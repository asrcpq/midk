fn main() {
	...
	let output = args.get("output")
		.map(|x| x[0].clone())
		.unwrap_or("/tmp/midk_audiorec.wav".to_string());
	...
	let audio_in1 = client.register_port("audio_in1", jack::AudioIn::default())
		.unwrap();
	let audio_in2 = client.register_port("audio_in2", jack::AudioIn::default())
		.unwrap();
